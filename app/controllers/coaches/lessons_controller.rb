class Coaches::LessonsController < Coaches::CoachesController
  ...
  def update
    @lesson = Lesson.find(params[:id])
    @lesson.update(update_lesson_params)
  ...
  def update_lesson_params
    ...
    params.require(:lesson).permit(:title, :genre_id, :thumbnail, :start_day, :month, :desc_goal, :desc_who, :desc_what, :desc_other, lesson_shifts_attributes: [:id, :weekday, :start_time, :finish_time]).merge(shift_minutes: shift_minutes, coach_id: current_user.id)
  end
